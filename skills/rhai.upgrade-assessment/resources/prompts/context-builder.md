...
Build the shared context file (`context.md`) that serves as the sole source of truth for all persona assessments. This step resolves repos, reads architecture docs, cross-validates CRDs against actual component code, reads odh-cli checks, and produces a verified resource inventory with discrepancies documented.
...
## Step 1: Resolve Versions and Update Repos
...
Create `discrepancies.yaml` at the start of Step 2 (empty list `[]`). To append a discrepancy: read the existing file, parse the YAML array, append the new entry, and rewrite the full file. Do not append raw text — that produces malformed YAML.
...
This is the most critical step. Every resource must be enumerated individually. Nothing may be skipped.
...
Also read the odh-gitops manifests. For each of **source** and **target** RHOAI versions, if a branch exists matching the version (e.g. `rhoai-3.3`), ensure a **dedicated** tree and read `components/operators/`, `dependencies/operators/`, and `configurations/` from that path only. The gitops repo is authoritative for dependency operators. If no matching branch exists, fall back to architecture-context and note the gap. Record each gitops path used and its HEAD SHA in `context.md`.
...
Build a unified resource map. For each resource discovered, record which file it was read from (repo, branch, path). This provenance feeds the **Source Map** section in `context.md`.
...
After computing the CRD diff, scan the results for changes that look anomalous. The following patterns are **red flags** that require mandatory verification against the actual component repo before being included in context.md. Do not include them as confirmed changes until verified.
...
### 2b-iii. Cross-Component Interaction Map
...
### 2b-iv. Accidental Data-Plane Restart Detection
...
### 2b-v. Dependency Operator Disruption Analysis
...
## Step 3: Read odh-cli Check Registry
...
## Step 4: Generate Run ID and Write context.md

Run ID: `{source}-to-{target}-{YYYYMMDD-HHMMss}` (e.g., `2.25-to-3.3-20260425-143022`)

Create directory: `.context/tmp/upgrade-assessments/{run_id}/`

Write `context.md` with all data from Steps 1-3. The file must contain:
