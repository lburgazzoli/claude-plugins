...
- Only report ADR-related findings when the upgrade transition itself introduces, changes, or breaks the ADR's implementation. For example: an ADR adopted in 2.20 and implemented in 2.25 is not a finding for a 2.25→3.3 upgrade, even if the architect persona encounters the ADR for the first time.
